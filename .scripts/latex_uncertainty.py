r"""
takes value-uncertainty pairs from csv (or similar formated file) and returns code for a latex table
-> $value \pm uncertainty$ & ... \\ \hline
    ...
    with open(csv_filepath, "r") as file:
        for line in file:
            ...
            for i in range(len(line_list)//2):
                string += rf"${line_list[2*i]}\pm" + str(line_list[2*i + 1].replace('\n', '')) + "$"
                # the '&' character should not be added after last column
                if i + 1 < len(line_list)//2:
                    string += "\t& "
            string += "\t" + r"\\ \hline" + "\n"
